  ...
  return (
    <Dialog title="清理 Gateway 日志" onClose={onClose} className="gateway-log-cleanup-dialog" busy={loading || deleting}>
      <div className="gateway-log-cleanup-form">
        ...
        <div className="gateway-log-cleanup-query-actions"><button type="button" className="secondary" onClick={applyQuery} disabled={loading || deleting}><Search size={15} aria-hidden="true" />查询</button><IconButton icon={RefreshCw} label="刷新查询结果" onClick={() => void refresh()} disabled={loading || deleting} className={loading ? "is-spinning" : ""} /></div>
      </div>
      ...
      <footer><button type="button" className="secondary" onClick={onClose} disabled={loading || deleting}>取消</button><button type="button" className="danger" onClick={() => void deleteSelected()} disabled={loading || deleting || selectedIds.size === 0}><Trash2 size={15} aria-hidden="true" />{deleting ? "删除中..." : `删除已选 (${selectedIds.size})`}</button></footer>
    </Dialog>
  );
}
